...
def quant_page(df):
    ...
    with st.container():
        ...
        with mod:
            model = st.selectbox("Model:", options=['LLaMA-3.1-70B-Instruct', 'Mistral-NeMo'], index=0, key='model')
        ...
        with guid:
            ...
            guided = st.selectbox('Select Guidance', ['Non-guided'], index=0)
        ...
        energy_bars = alt.Chart(df_transformed).mark_bar(color=energy_color, opacity=0.6, size=20).encode(
            ...
            tooltip=[
                alt.Tooltip('quantization:N', title='Quantization'),
                alt.Tooltip('energy_consumed:Q', title='Energy Consumed (kWh)')
            ]
        )
